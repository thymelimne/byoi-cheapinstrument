...
class EnvelopeDrawing(Screen):
	...
	#The following are just to assist the app in reading keystrokes:
	...
	def _on_keyboard_down(self, keyboard, keycode, text, modifiers):
		print(keycode[1])
		sm.switch_to(display)
	
class DisplayEnvelope(Screen):
	pass
...
class TextField(TextInput):
	...
	#The following is just to assist the app in reading keystrokes:
...
if __name__=="__main__":
	class DrawingApp(App):
		def build(self):
			global sm #Made into a global variable as a shortcut, to be able to change screens from anywhere in the code.
			global drawing
			global display
			sm = ScreenManager(transition=FadeTransition())
			drawing = EnvelopeDrawing(name="drawing")
			display = DisplayEnvelope(name="display")
			sm.add_widget(drawing)
